...
export function HouseholdSection({ value, onChange, errors, action, number }: Props) {
  const t = useTranslations("Profile");
  const tp = useTranslations("PetType");
  const set = <K extends keyof Household>(k: K, v: Household[K]) => onChange({ ...value, [k]: v });

  return (
    <FormSection number={number} title={t("household")} hint={t("householdHint")} action={action}>
      ...
      <div className="flex flex-col gap-2">
        <p className={fieldLabelClass}>{t("kids")}</p>
        {value.kids.map((k, i) => (
          <div key={i} className="flex items-end gap-2">
            ...
            />
            <Button type="button" variant="ghost-destructive" size="xs" className="mb-2.5" onClick={() => set("kids", value.kids.filter((_, j) => j !== i))}>
              {t("remove")}
            </Button>
          </div>
        ))}
        <Button type="button" variant="outline" size="xs" className="self-start" disabled={value.kids.length >= 10} onClick={() => set("kids", [...value.kids, { age: 5 }])}>
          <span aria-hidden>+</span> {t("addKid")}
        </Button>
      </div>

      <div className="flex flex-col gap-2">
        <p className={fieldLabelClass}>{t("pets")}</p>
        {value.pets.map((p, i) => (
          <div key={i} className="flex items-end gap-2">
            <SelectField
              className="flex-1"
            ...
            />
            <Button type="button" variant="ghost-destructive" size="xs" className="mb-2.5" onClick={() => set("pets", value.pets.filter((_, j) => j !== i))}>
              {t("remove")}
            </Button>
          </div>
        ))}
        ...
        >
          <span aria-hidden>+</span> {t("addPet")}
        </Button>
      </div>
    </FormSection>
  );
