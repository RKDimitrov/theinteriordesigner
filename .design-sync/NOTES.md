...
- The design-system check also scans `_ds_bundle.css`. It flagged the Tailwind `@layer theme` copies of those tokens and the `--tw-*` declarations under utility selectors (`--tw-translate-x/y`, `--tw-border-style`, `--tw-outline-style`, `--tw-backdrop-blur`, `--tw-duration`) as unlabelled. So `build-ds.mjs` step 6 rewrites `dist/styles.css` (which ships as `_ds_bundle.css`): unclassified tokens in `:root` / `:root, :host` / `.dark` blocks, and `--tw-*` declarations, get `/* @kind other */` (65 tags as of this writing). The theme block is tagged, not removed, so the bundle CSS stays self-sufficient.
- **Scope rule (from the check's feedback):** a tag only attaches when its rule sits directly in an `@layer` (or at top level for theme blocks). Tags inside nested `@supports`/`@media` blocks (e.g. the `.ring-*` fallbacks, the `@layer properties` `@supports` copy of `--shimmer-angle`), inside nested rules, or in combinator selectors (`>`, `:is(`, `:where(`) are reported as stray, so step 6 skips those scopes.
